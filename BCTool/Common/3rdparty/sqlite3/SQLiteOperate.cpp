...
void SQLiteOperate::InitDb()
{
	...
	PathAppend(szDbPath, _T("data\\post.db"));
	...
	if (pBuf != NULL)
	{
		...
		if (!IsExistTable("RankData"))          //初始化表格
		{
			InitTables();
		}
...
void SQLiteOperate::SaveUserInfo(UserInfo const &userInfo )
{
	try
	{
		...
		ostr << "SELECT UserName,PassWord  FROM UserInfo where UserName='" << userInfo.szUName << "'";
		strSql = ostr.str();
		//strSql.Format("SELECT UserName,PassWord  FROM UserInfo where UserName='%s'", userInfo.szUName);

		if (SelectStmt(strSql, restTable))
		{
			if (restTable.next() != NULL)
			{
				ostr << "update UserInfo set PassWord='" << userInfo.szPwd << "',SavePwd=" << userInfo.iSavePwd << ",AutoLogin=" 
					<< userInfo.iAutoLogin << ",ProDuctId=" << userInfo.iProduceId << "  where UserName='" << userInfo.szUName << "'";
...
// 					, userInfo.szUName);
			...
			{
				ostr << "insert into UserInfo(UserName,PassWord,SavePwd,AutoLogin,ProDuctId) values('" << userInfo.szUName << "','" << userInfo.szPwd
					<< "'," << userInfo.iSavePwd << "," << userInfo.iAutoLogin << "," << userInfo.iProduceId;
...
bool SQLiteOperate::GetUserInfo(UserInfo &userInfo )
{
	try
	{
		...
		std::ostringstream ostr;
		ostr << "SELECT UserName,PassWord,SavePwd,AutoLogin,ProDuctId  FROM UserInfo";
		strSql = ostr.str();
