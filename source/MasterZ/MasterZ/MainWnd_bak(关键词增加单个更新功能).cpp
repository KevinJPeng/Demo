...
{	 
	...
	//加载第一个图表网页
	m_pActiveXFlash = static_cast<CWebBrowserUI*>(m_PaintManager.FindControl(_T("flash_keywordrank")));
...
//	m_pActiveXFlash->SetVisible(false);
	if (m_pActiveXFlash != NULL)
	{
		...
		m_pActiveXFlash->Navigate2(m_strEmptyKeywordPath.GetBuffer());
	}
...
{	
	...
	{	
		...
		{	
			if (m_dwClientType == CUSTOMIZE_VERSION)
			{	
...
{	
	...
	{	
		...
		//更新关键词
		if (msg.pSender->GetName().Find(_T("rank_update")) != -1)
		{
			::MessageBox(NULL, _T("abc"), msg.pSender->GetName(), MB_OK);
		}
...
{	
	...
	for(int i = 0; i < 10; i++)
	{
		...
		int nCount = 0;
		iniFile.ReadInteger(strField, _T("count"), nCount);
		iniFile.ReadString(strField, _T("file"), strFile);
...
{	
	...
	//设置定时器,1.5min中之后首次请求最新推送消息----新修改为30s
	::SetTimer(m_hWnd,ID_TIMING_GET_MESSAGE,30*1000,NULL);
...
void CMainWnd::StartReFreshKeyWordRank (WORD wSendkeyWordFlag)
{
	...
	PackagMessage(E_THREAD_AUTO_REFRESH_KEYWORD,E_THREAD_TYPE_UIHELPER,MSG_AUTO_REFRESH_KEYWORD,dwRefreshKeyWord,(LPARAM)&m_tUserInfoFreshWord);
...
void CMainWnd::SendConnectSerMsg()
{
	...
	PackagMessage(E_THREAD_AUTO_REFRESH_KEYWORD,E_THREAD_TYPE_UIHELPER,MSG_CONNECT_SERV_WITH_UDP,0,(LPARAM)&m_tConnectServer);
...
{	
	...
	CIniFile iniFile((CStdString)strCfgFile);
	iniFile.ReadString(_T("product"), _T("Name"), strWindowsName);
...
{	
	...
	for (int m = 0; m <= 7; ++m)
	{
		...
		str.Format(_T("rank_update%d"), m);
		pButton = (CButtonUI*)m_PaintManager.FindControl(str);
		m_vecRankButton.push_back(pButton);
	}
	for (int n = 0; n <=7; ++n)
	{
	...
	case RANKUI_RUNNING:		//正在获取状态
		{
			...
			PLable =  (CLabelUI*)m_PaintManager.FindControl(_T("rank_getrunning"));
...
{	
	...
	{	
		...
		{	
			...
			m_vecRankButton[n*2+0]->SetVisible(true);
			m_vecRankButton[n*2+0]->SetEnabled(false);
			m_vecRankButton[n*2+1]->SetVisible(true);
			m_vecRankButton[n*2+1]->SetEnabled(false);
		}
		else
		{
			...
			m_vecRankButton[n*2+0]->SetVisible(true);
			m_vecRankButton[n*2+0]->SetEnabled(true);
			m_vecRankButton[n*2+1]->SetVisible(true);
			m_vecRankButton[n*2+1]->SetEnabled(true);
		}
		//日期
...
{	
	...
	if (m_bIsNewestVersion && bIsUserManualLogin)
	{
		...
		PackagMessage(E_THREAD_AUTO_REFRESH_KEYWORD,E_THREAD_TYPE_UIHELPER,MSG_AUTO_REFRESH_KEYWORD,dwRefreshKeyWord,(LPARAM)&m_tUserInfo);
...
{	
	...
	if (wParam == WPARAM_POWER_ERROR && m_dwClientType != CUSTOMIZE_VERSION)
	{
...
{	
	...
	{	
		...
		if (wParam == 1)
		{
			...
			PackagMessage(E_THREAD_AUTO_REFRESH_KEYWORD, E_THREAD_TYPE_UIHELPER, MSG_CHECK_REFRESH_KEYWORD_STATA, 0, 2);
		}
		else
		{	
	...
	{	
		...
		{	
			//此时发送检测排名模块状态消息
			PackagMessage(E_THREAD_AUTO_REFRESH_KEYWORD, E_THREAD_TYPE_UIHELPER, MSG_CHECK_REFRESH_KEYWORD_STATA, 0, 1);
		}
	}
	m_bIsRankRunning = false;
...
afx_msg void  CMainWnd::OnHandleServerMsg(WPARAM wParam, LPARAM lParam)
{
	...
	else if (!strInfo.Compare(_T("CMD_SUBMIT_LOG")))
	{
		...
		PackagMessage(E_THREAD_AUTO_REFRESH_KEYWORD,E_THREAD_TYPE_UIHELPER,MSG_SUBMIT_LOG,0,(LPARAM)&m_tConnectServer);

		g_log.Trace(LOGL_TOP,LOGT_PROMPT, __TFILE__,__LINE__, _T("提交日志消息已经由界面发出!"));
	}
	else if(!strInfo.Compare(_T("CMD_SUBMIT_LOGANDPHOTO")))
	{
		PackagMessage(E_THREAD_AUTO_REFRESH_KEYWORD,E_THREAD_TYPE_UIHELPER,MSG_SUBMIT_LOG,1,(LPARAM)&m_tConnectServer);
...
//		m_showchart.LoadExposureCountChart(m_tDataSets.productList);
	...
	if (msg.pSender->GetName() == _T("option_keywordrank1") || msg.pSender->GetName() == _T("option_keywordrank"))
	{
		...
		{	
			...
			ShowRankUiByState(RANKUI_RUNNING);
			PackagMessage(E_THREAD_AUTO_REFRESH_KEYWORD,E_THREAD_TYPE_UIHELPER,MSG_CHECK_REFRESH_KEYWORD_STATA,0,0);
