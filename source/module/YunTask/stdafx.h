...
struct T_LIMIT_REQUEST_COUNT
{	
	//各类型任务失败次数
	DWORD m_dwKwTaskErrorCount;
	DWORD m_dwShopTaskErrorCount;
	DWORD m_dwFreshTaskErrorCount;
	DWORD m_dwMainTaskErrorCount;

	//当前已经等待任务轮数
	DWORD m_dwKeyWordCurrWaitRound;
	DWORD m_dwShopCurrWaitRound;
	DWORD m_dwFreshCurrWaitRound;
	DWORD m_dwMainTaskCurrWaitRound;

	//失败后需要等待的任务总轮数
	DWORD m_DwKeyWordErrorWaitRound;
	DWORD m_DwShopErrorWaitRound;
	DWORD m_dwFreshErrorWaitRound;
	DWORD m_dwMainTaskWaitRound;

	T_LIMIT_REQUEST_COUNT()
	{	
		m_dwKwTaskErrorCount = 0;
		m_dwShopTaskErrorCount = 0;
		m_dwFreshTaskErrorCount = 0;
		m_dwMainTaskErrorCount = 0;

		m_dwKeyWordCurrWaitRound = 0;
		m_dwShopCurrWaitRound = 0;
		m_dwFreshCurrWaitRound = 0;
		m_dwMainTaskCurrWaitRound = 0;

		m_DwKeyWordErrorWaitRound = 1;
		m_DwShopErrorWaitRound = 1;
		m_dwFreshErrorWaitRound = 1;
		m_dwMainTaskWaitRound = 1;
	}
...
extern CLogTrace g_logYunSuccAccount;

extern CLogTrace g_YunDataLog;
extern CLogTrace g_SubmitLog;

extern IXMLRW g_iniCfgFile;
extern CIniFile g_iniDebugData;
extern CLock g_lock;

// 任务类型Cstring到int类型映射
extern std::map<CString, int> g_TypeMap;
extern bool g_bDelFile;
