...
CLogTrace g_commLog(_T("TaskComm.log"),NULL);
CLock g_lock;
...
//连接到服务端
int  CCommClient::Init(int iPort, CTaskBase *pBase)
{
...
// 判断接受线程是否结束 add by zhoulin接受线程结束后会释放资源
void CCommClient::WaitReceiveExit()
{
	if (m_hThread != NULL)
	{
		DWORD dwExitCode = 0;
...
DWORD CCommClient::OnReceive(char *Orgbuf, DWORD dwTotalLen)
{
	...
	if (!m_bStop)
	{
		...
		if (m_pTaskBase != NULL)
		{	
			m_pTaskBase->OnReceive(strData);
			CString strTag = strData;
			if (strTag.Find(_T("BackResult(;0)AllTaskComplete")) == 0 || 0 == strTag.CompareNoCase(_T("DISCONNECTED")))
			{
				// 发送通告清理任务内存
				m_pTaskBase->ReleaseTask();
			}
		}
		g_lock.Unlock();
	}
