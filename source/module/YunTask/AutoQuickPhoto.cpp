...
CAutoQuickPhoto::CAutoQuickPhoto(CTaskThread *pTaskThread, CAutoGetKeywordPages *pAutoGetKeywordPages)
{
	//m_hProcess = NULL;
...
bool CAutoQuickPhoto::Init(void)
{
	...
	strInitPath.Format(_T("%s\\cfg\\mcconfig.ini"), g_pGlobalData->dir.GetInstallDir());
...
void CAutoQuickPhoto::Stop(void)
{
	...
	//Sleep(200);  //因为TerminateProcess是异步的，防止comm中m_pTaskBase->OnReceive(strData)时m_pTaskBase被同时置为null
	m_comm.Stop();
}
...
bool CAutoQuickPhoto::SendResultToServer(void)
{
	...
	strPostData.Format(_T("%s||%s||%s||%s||%s||%s"), m_tTaskData.strUID, m_tTaskData.strUIPdtVer, \
		strCacheData, m_pTaskThread->GetClientMacAddr(), strStaticInfo, m_tTaskData.strExtraData);
...
Retry:
	...
	//清空缓存结果
	//WritePrivateProfileString(_T("Task"), m_tTaskData.strSubject + m_tTaskData.strUID, _T(""),  m_tszDataCacheFile);
	/*if (m_nJudgeWebcDisconnected == 1)
	{
...
NotUploadZip:
		...
		strFileName.Format(_T("%s_keyword_%s\.zip"), m_tTaskData.strCUID, m_pTaskThread->GetClientMacAddr());
...
//通知任务结束的回调函数
void CAutoQuickPhoto::SetTaskMgr(CTaskMgr *pMgr)
{
	m_pMgr = pMgr;
}
...
void CAutoQuickPhoto::OnReceive(CString strData)
{
	...
	if (strData.Find(_T("BackResult(;0)AllTaskComplete")) == 0 || 0 == strData.CompareNoCase(_T("DISCONNECTED")))
	{
		...
		if (0 == strData.CompareNoCase(_T("DISCONNECTED")) && !g_bWebcDisconnected && !m_bStopWebc)
		{
			...
			m_pTaskThread->HandleRefreshKeyword();
			return;
		}
...
void CAutoQuickPhoto::GetUploadFilePath(CString &strUploadPath, CString strMode)
{
	...
	if (strMode == _T("Zip"))
	{
		strMacAddr = m_pTaskThread->GetClientMacAddr();
