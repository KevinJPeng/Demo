...
int CInternetHttp::ExecuteRequest(LPCTSTR strMethod, LPCTSTR strUrl, LPCTSTR strPostData, CString &strResponseText, DWORD codeType /*= CP_UTF8*/)
{
    CString strServer;
    CString strObject;
    DWORD dwServiceType;
    ...
    try
    {
		...
		//UTF8转宽字节
		dwSize = 0;
		MByteToWChar((LPSTR)(strRawResponse.c_str()), NULL, &dwSize, codeType);
		WCHAR *pwszBuf = new WCHAR[dwSize];
		MByteToWChar((LPSTR)(strRawResponse.c_str()), pwszBuf, &dwSize, codeType);
...
int CInternetHttp::HttpGet(LPCTSTR strUrl, LPCTSTR strPostData, CString &strResponse, DWORD codeType /*= CP_UTF8*/)
{
    return ExecuteRequest(_T("GET"), strUrl, strPostData, strResponse, codeType);
}
...
// int CInternetHttp::HttpPost(LPCTSTR strUrl, char* pstrPostData, CString &strResponse)
// {
// 	return ExecuteRequest(_T("POST"), strUrl, pstrPostData, strResponse);
// }

// int CInternetHttp::ExecuteRequest(LPCTSTR strMethod, LPCTSTR strUrl, char* pstrPostData, CString &strResponseText, DWORD codeType /*= CP_UTF8*/)
// {
// 	CString strServer;
// 	CString strObject;
// 	DWORD dwServiceType;
...
// 		m_pFile->AddRequestHeaders(_T("Accept-Language: zh-cn"));
// //		m_pFile->AddRequestHeaders(_T("Content-Type: application/x-www-form-urlencoded; charset=utf-8"));
// 		m_pFile->AddRequestHeaders(_T("Content-Type: multipart/form-data; boundary=----WebKitFormBoundarysjO6SEctMcjegKml"));  //统一用utf-8防止乱码
// 		//m_pFile->AddRequestHeaders(_T("Accept-Encoding: gzip, deflate"));
// 
// 		//刷排名提交任务容易超时，设置超时为10分钟
// 		m_pFile->SetOption(INTERNET_OPTION_SEND_TIMEOUT, 10 * 60 * 1000, 0);
...
// 		char *pszBuf = NULL;
// 		BOOL bRes = FALSE;
// 		if (pstrPostData != NULL)
// 		{
// // 			WCharToMByte((LPWSTR)(strPostData), NULL, &dwSize, CP_UTF8);
// // 			char *pszBuf = new char[dwSize];
// // 			WCharToMByte((LPWSTR)(strPostData), pszBuf, &dwSize, CP_UTF8);
// 			bRes = m_pFile->SendRequest(NULL, 0, pstrPostData, dwSize);
// //			delete[]pszBuf;
// 		}
// 		else
// 		{
...
void CInternetHttp::UploadFile(const CString& strFileURLInServer, const CString& strFileLocalFullPath, CString &strResponse)
{
	...
	try 
	{
		...
		if (NULL == m_pConnection)
		{
			strResponse = _T("GetHttpConnection fail");
			return;
		}
