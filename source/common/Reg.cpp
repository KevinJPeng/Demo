...
//         _T("在注册表中读取键值"));
	...
	else if (lRes == ERROR_FILE_NOT_FOUND && hKey == HKEY_CURRENT_USER)
	{
		RegCloseKey(hKey);
		TCHAR *szUsers = GetUserRegKey();
		hKey = HKEY_USERS;
		CString strNewPath;
		strNewPath.Format(_T("%s\\%s"), szUsers, pszKeyPath);
		long lRes = RegOpenKeyEx(hKey, strNewPath, 0, KEY_ALL_ACCESS, &hSubKey);
		if (lRes == ERROR_SUCCESS)
		{
			RegQueryValueEx(hSubKey, pszKeyName, 0, &dwType, pbyValue, &dwLen);
...
//取HKEY_USERS下面当前用户的键名
TCHAR* CReg::GetUserRegKey() const
{
	HKEY hKey;
	int i = 0;
	TCHAR *pszKeyName = NULL;
	long lRes = RegOpenKeyEx(HKEY_USERS, _T(""), 0, KEY_ALL_ACCESS, &hKey);
	if (lRes == ERROR_SUCCESS)
	{
		...
		{
			pszKeyName = new TCHAR[MAX_KEY_LENGTH];
			long lSubRes = RegEnumKey(hKey, i++, pszKeyName, MAX_PATH);
			if (lSubRes != ERROR_SUCCESS)
			{
				delete[]pszKeyName;
				break;
			}
			if (_tcsstr(pszKeyName, _T("S-1-5-21-")) != NULL
				&& _tcsstr(pszKeyName, _T("Classes")) == NULL)
			{
				RegCloseKey(hKey);
				return pszKeyName;
			}
