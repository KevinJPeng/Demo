...
void CLogTrace::OpenLogFile(const TCHAR * pszTraceFileName, HWND hWnd, BOOL bNeedZip)
{
	...
	IXMLRW xmlCfg;
	int irecode = xmlCfg.init(tszTmp);
...
void CLogTrace::TraceSim(BYTE cLevel, LPCTSTR lpszFormat, ...)
{
    ...
    //打印
    TraceColor(cLevel, LOGC_PROMPT,FALSE);
}
...
///////////////////////////////////////////////////////////////////////////////
void CLogTrace::TraceColor(BYTE cLevel, DWORD dwColor, BOOL bIsEncrypt)
{
	if(IsLogFileEnable()==FALSE)return;
    switch (cLevel)
    {
    ...
    case LOGL_LOW:
        {
			...
			if (m_hLogFile) //Log文件可用
            {
                ...
                if (0 == s_dwWriteCount % MAX_CHK_SIZE_PER_TIMES)
                {
                    ...
                    {
// 						CString strJiNuo(_T("UpdateRank.log"));
// 						CString strFileName = m_szFullLogFileName;
// 						if (strJiNuo == strFileName.Right(14))
// 						{
// 							CloseHandle(m_hLogFile);
// 							m_hLogFile = NULL;
// 
// 							string strPathOld = CStringToString(strFileName);  //CString->string
// 							string strPathNew;  //CString->string
// 
// 							int iPos = strFileName.ReverseFind(_T('\\'));
// 							strFileName = strFileName.Left(iPos +1);
// 							for (int i = 0; i < 1000; i++)
// 							{
// 								strJiNuo.Format(_T("UpdateRank%d.log"), i);
// 								strFileName += strJiNuo;
...
// 								}
// 							}
// 							strPathNew = CStringToString(strFileName);
// 							rename(strPathOld.c_str(), strPathNew.c_str());
// 							return;
// 						}
// 						else
// 						{
...
// 						}
				...
				CString strEncrypt = _T("");  //加密之后

				//加密日志信息 begin--
				strEncrypt = m_szBuffer[1];
				if (IsEncryptLog() && bIsEncrypt)
				{					
					m_key.EncryptData(m_szBuffer[1], LOG_ENCRYPT_KEY, strEncrypt);
					
				}
				//--end

#ifndef _MBCS
				pMultipleStr = ToMultiByte(strEncrypt.GetBuffer(), &nWriteStrLen);				
...
// std::string CLogTrace::CStringToString(CString cstr)
// {
// 	std::string str;
