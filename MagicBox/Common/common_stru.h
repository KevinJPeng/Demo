...
struct T_TaskDetailInfo
{
	...
	TCHAR szShortSubject[MAX_NAME_LENGTH];					//发布简略主题
	TCHAR szPostColumn[MAX_MID_LENGTH];						//发布栏目	
	
	TCHAR szNewsTitle[MAX_NAME_LENGTH];					//新闻标题
	TCHAR szNewsInfo[MAX_CONTEXT_LENGTH];				//新闻内容
    ...
    T_TaskDetailInfo()
    {
		...
		ZeroMemory(szShortSubject, sizeof(szShortSubject));
		ZeroMemory(szPostColumn, sizeof(szPostColumn));		
		ZeroMemory(szNewsTitle, sizeof(szNewsTitle));
		ZeroMemory(szNewsInfo, sizeof(szNewsInfo));
    }
};
...
struct T_CompanyInfo
{
	...
	//建站信息
	TCHAR szHomepageTemp[MAX_MID_LENGTH];				//主页模版
	TCHAR szWebName[MAX_NAME_LENGTH];					//网站名称
	TCHAR szTempletStyle[MAX_MID_LENGTH];				//模版风格
	TCHAR szCopyright[MAX_NAME_LENGTH];					//网站版权
	TCHAR szSiteDesc[MAX_SUBJECT_LENGTH];				//站点描述
	TCHAR szSiteKeyword[MAX_NAME_LENGTH];				//站点关键字

	TCHAR szColumns[MAX_NAME_LENGTH];					//网站栏目
	TCHAR szSubjectColumn[MAX_MID_LENGTH];				//隶属栏目
	TCHAR szPostTemple[MAX_MID_LENGTH];					//内容模型
	TCHAR szDefaultPage[MAX_MID_LENGTH];				//默认页名称
	TCHAR szTagFlag[MAX_NAME_LENGTH];					//Tag标签
    ...
    T_CompanyInfo()
    {
		...
		ZeroMemory(szHomepageTemp, sizeof(szHomepageTemp));
		ZeroMemory(szWebName, sizeof(szWebName));
		ZeroMemory(szTempletStyle, sizeof(szTempletStyle));
		ZeroMemory(szCopyright, sizeof(szCopyright));
		ZeroMemory(szSiteDesc, sizeof(szSiteDesc));
		ZeroMemory(szSiteKeyword, sizeof(szSiteKeyword));
		ZeroMemory(szColumns, sizeof(szColumns));
		ZeroMemory(szSubjectColumn, sizeof(szSubjectColumn));
		ZeroMemory(szPostTemple, sizeof(szPostTemple));
		ZeroMemory(szDefaultPage, sizeof(szDefaultPage));
		ZeroMemory(szTagFlag, sizeof(szTagFlag));
    }
};
...
struct T_SaveToServerAccount
{
	...
	int dwCompanyId;					//公司ID 用于和八方资源网对接用的
