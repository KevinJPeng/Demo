...
CUtility::CUtility(IObserver* _obs)
{
	m_pobs = _obs;
}
CUtility::~CUtility()
				...
				if ((iSecondS > iSecondC) || (iSecondS == iSecondC && iThreeS > iThreeC))
				{
					...
					if (0 == curlAPIObj.DLFileByHttp(WideToChar(sServerUrl.c_str()), WideToChar(sLocalPathName.c_str())))
					{
						...
						if (fileOpeObj.IsPathExistEx(sLocalPathName))
						{
							...
							{
								LOG_INFO(g_logger, "MD5比对失败，下载的文件与服务器上versioncfg.xml的MD5不一致");
							}
						}
					}
					else
					{
						LOG_INFO(g_logger, "软件升级失败");
					}
				}
			}
		}
...
int CUtility::DLAndInstallClient()
{
	...
	LOG_INFO(g_logger, "当前版本:" << WideToChar(g_sVersion) << "&" << "服务器信息:" << pRecv);
	...
	{
		LOG_INFO(g_logger, "检测到用户已安装客户端");
		//启动客户端
		int iErrorCode = startClient(sClientName);
		LOG_INFO(g_logger, "启动客户端,Code = " << iErrorCode);
		return iErrorCode;
	}
	...
	if (fileOpeObj.IsPathExistEx(sLocalPathName))
	{
		LOG_TRACE(g_logger, "检测到安装包已存在：" << WideToChar(sLocalPathName.c_str()));
		//先关闭进程（进程若在运行中，下载会失败）
		CloseProcess("MasterZ_Custom_93.exe");
		string sMD5 = md5file(WideToChar(sLocalPathName.c_str()));
		if (sMD5 != m_serverInfo.sAPKMD5)
		{
			LOG_TRACE(g_logger, "本地安装包和服务器安装包MD5不一致：LocalMD5:" << sMD5 << "_ServerMD5:" << m_serverInfo.sAPKMD5);
			//删除已有安装包
			remove(WideToChar(sLocalPathName.c_str()));
		}
		else
		{
			LOG_TRACE(g_logger, "本地安装包和服务器安装包MD5一致,直接安装");
			bDL_APK = false;
		}
	}
	...
	if (bDL_APK)
	{
		...
		if (0 != curlAPIObj.DLFileByHttp(WideToChar(sServerUrl.c_str()), WideToChar(sLocalPathName.c_str())))
		{
			...
			LOG_INFO(g_logger, "安装包下载失败");
			return CLIENT_DL_FAIL;
		}
		LOG_INFO(g_logger, "安装包下载成功");
		//curlAPIObj.Detach(m_pobs);
	}
	...
	{
		LOG_INFO(g_logger, "安装包安装失败");
		return CLIENT_INSTALL_FAIL;
	} 

	LOG_INFO(g_logger, "安装包安装成功");
